...
// The global mutex
SemaphoreHandle_t g_serial_write_mutex;
#define MAX_WAIT_TICKS 30000	// wait in the semaphore.
...
static void serial_write_lab3 (const char *user_buf) {
    // Attempt to take the mutex, and block until we succeed.
    if (xSemaphoreTake(g_serial_write_mutex, MAX_WAIT_TICKS) != pdTRUE){
	error_152 ("Timed out trying to take the mutex");
    }
    ...
    // Return the mutex.
    if (xSemaphoreGive(g_serial_write_mutex) != pdTRUE)
	error_152 ("Illegal semaphore?");
}
...
int main(void){
    ...
    if ((g_serial_write_mutex=xSemaphoreCreateMutex()) == NULL)
	error_152 ("Cannot create semaphore");
